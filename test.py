...
x_data = np.linspace(-1,2,300)[:,np.newaxis]       #这个x数据有300行
...
l1 = add_layer(xs,1,200,activation_function=tf.nn.sigmoid)     #含义是输入数据为xs，该输入数据维度为1，隐藏层有10个神经元，因此输出维度为10激活函数为relu
prediction = add_layer(l1,200,1,activation_function=None)   #输入数据为隐藏层l1输出的数据，该数据由于是10个神经元发出，因此数据维度为10，而输出的为y所以输出唯独为1
